 ...
 * !!! WARNING !!!
 * All useful implementations of this class are already implemented. If you intend to write a thread safe method, please have a look at Void, VoidArgs, Ret and RetArgs.
 * This class is a template for other classes.
 * 
 * Objects of any type of ThreadSafeMethod are supposed to replace normal methods in the code. By overwriting the run method and starting it with the start method, the ThreadSafeMethod works as a wrapper to prevent multiple threads from altering the same fields.
 *
 */
abstract class ThreadSafeMethod {
	public static final class FieldList implements Iterable<LockManager.FieldInfo> {
		...
		private final Hashtable<LockManager.FieldInfo, Boolean> _restorationList = new Hashtable<LockManager.FieldInfo, Boolean>();
		...
		private void sort() {
			_list.sort(new Comparator<LockManager.FieldInfo>() {
				...
				@Override
				public int compare(LockManager.FieldInfo arg0, LockManager.FieldInfo arg1) {
	...
	protected final FieldList pre(Instant pInst)
